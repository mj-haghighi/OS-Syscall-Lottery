...
void itoa(char* string, int num);
void getArgs(char* args[10],int *args_count,int num)
{
  int temp_int;
  // char temp_string[10];
  switch (num) {
...
// put time here?
  ...
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
    ...
    // if (num==SYS_read)
    // {
      getArgs(args, &arg_c, num);
      register_syscall(curproc->pid, num, getName(num), args, &arg_c);
    // }
  } else {
